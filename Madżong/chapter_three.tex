...
\label{cc}
%\subsection{Historia}
Po okresie przekształcania licznych gier karcianych w pierwsze postaci madżonga
na przełomie XIX i XX wieku (patrz: rozdzial \ref{historia}), ze względu na brak
spisanych i ustalonych reguł występowało wiele rozbieżności w kwestii zasad
stosowanych w różnych regionach Chin i świata.
Pierwsze próby standaryzacji nastąpiły w latach dwudziestych XX wieku w
Szanghaju (patrz: strona \pageref{shanghai20s}). Naturalnie, pomiędzy
...
Wyjaśnienie najważniejszych cech szczególnych zasad chińskich klasycznych w
...
Madżonga'' (Babcock \& Foster \& Hartman \& Smith \& Work 1924), nazywanych w
dalszej części tej sekcji (zgodnie z nazewnictwem stosowanym w kodeksie)
\foreign{Mixed-Hand}, \foreign{One-Double} oraz
\foreign{Cleared-Hand}\footnote{Nazwy 3 wymienionych podwariantów pochodzą od
wymagań narzuconych dla wygrywającej ręki w każdym z nich. \foreign{Mixed-Hand}
to dosłownie ,,mieszana ręka'' (brak restrykcji względem używanych kamieni),
\foreign{One-Double} to ,,jedno podwojenie'' (jeden obowiązkowy mnożnik na
wygrywającej ręce), natomiast \foreign{Cleared-Hand} -- ,,wyczyszczona ręka''
(wymagana jest czysta ręka składająca się z tylko jednego typu kamieni oraz
opcjonalnie honorów).}.
...
Zamiast stosowanej w przypadku międzynarodowych zasad turniejowych rotacji
miejsc, gracze przekazują wiatr miejsca w kierunku przeciwnym do wskazówek
...
rozdanie (innymi słowy, runda składa się ze zmiennej liczby rozdań -- 4 +
liczba zwycięstw wiatru wschodniego w tej rundzie).
...
ilości różnych fan, jakie naraz mogą znaleźć się w tym wariancie na wygrywającej
ręce, wysokie wygrane mogły potencjalnie przekraczać możliwości finansowe
przegranych uczestników gry. Z tego właśnie względu wprowadzono \jyutping{laat},
które pozwalają ograniczyć maksymalną możliwą wygraną. Fan o łącznej wartości 4,
5 lub 6 mają wartość 1 \jyutping{laat}, które przekłada się na 16 punktów
(czyli standardową wartość dla 4 fan). 2 \jyutping{laat} to od 7 do 9 fan,
natomiast 3 \jyutping{laat} to 10 i więcej. W bardziej hazardowym podwariancie
można grać aż do 5 \jyutping{laat} (20 fan), w czego efekcie wartość ręki może
osiągnąć nawet 2048 punktów, przez co przed grą istotne jest ustalenie przez
graczy, do którego limitu zamierzają grać (Rep 2006).
...
skomplikowany. Ponadto, Cofa Tsui argumentuje, że jest mało prawdopodobnym, by w
...
się z nią w Chinach, a następnie w roku 1924 założył szkołę gry (w tym samym
roku miały miejsce pierwsze zawody na skalę ogólnokrajową). Według innej teorii
madżong dotarł do Chin za pośrednictwem pisarza Natsume Sōseki \footnote{Natsume
Sōseki (夏目漱石, 1867-1916) -- jego prawdziwe imię to Kinnosuke (金之助); pisarz
...
którym organizowane są turnieje Europejskiego Organizacji Mahjonga (drugi to
...
także w budowie martwego muru, jest \romaji{dora} (ドラ) -- specjalny, losowowy w
...
inne), jednakże pozostałe elementy gry są batdzo zbliżone do madżonga
...
często nie jest tak opłacalne, jak byłoby w innych wariantach zasad.
